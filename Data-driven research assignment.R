...
##Defined "majority" as 75% rather than 50% to be more conservative; policy makers, for example, may want to base
###predictions of higher %% because certain measures may need more than 50% of voters in favor to pass, and not all voters show up to 
##the poll; choosing a higher value to qualify as "majority" is a practical decision
state.data <- mutate(state.data, maj.regulate = ifelse(regulate > 75, 1,0))


#Subsequen Analysis will create a logistic regression model that might hypothetically be used to predict likelihood that a state will "vote" to regulate based on
